...
# Reduce columns
SemanticJoinFile = SemanticJoinFile[["CUI", "SemanticType"]] # , "SemanticTypeAbr"
...
umlsTermList = umlsTermList.sort_values(by='wordCount', ascending=False).reset_index(drop=True)

# Some UMLS terms are super long and I don't think people are using these in
# site search, so, cutting off
...
# Dupe off PreferredTerm column so we can tinker with the dupe
umlsTermList['AdjustedQueryTerm'] = umlsTermList['PreferredTerm'].str.lower()
# Remove anything that's not a letter or a number

# Replace hyphen with space because the below would replace with nothing, merging words
umlsTermList['AdjustedQueryTerm'] = umlsTermList['AdjustedQueryTerm'].str.replace('-', ' ')

# Remove all chars except a-zA-Z0-9 and leave foreign chars alone
# https://docs.python.org/3/library/re.html
umlsTermList['AdjustedQueryTerm'] = umlsTermList['AdjustedQueryTerm'].str.replace(r'[^\w\s]+', '')

umlsTermList.columns
'''
'PreferredTerm', 'ui', 'LAT', 'SAB', 'SemanticType', 'wordCount',
       'AdjustedQueryTerm'
'''

# Re-order
umlsTermList = umlsTermList[['AdjustedQueryTerm', 'PreferredTerm', 'SemanticType', 'wordCount', 'ui', 'LAT', 'SAB']]

# Correct result? Eyeball.
viewumlsTermList = umlsTermList[10000:10500]
viewumlsTermList.head()
...
umlsTermList.to_csv(dataMatchFiles + 'UmlsMesh.csv', 
                               sep='|', encoding='utf-8',
...
del [[atomForCui, SemanticJoinFile, umlsTermList]]
